...
while True:
    ...
    click_pause_user(button_comenzar)
    ...
    text_lives = font.render(f"Lives: {lives}", True, MAGENTA)
    text_rect_lives = text_lives.get_rect(topright = (WIDTH - 30, 40))
    # text_rect.midtop = (WIDTH // 2 , 30)
    #game_time = FPS * 30
    is_runnig = True
    pygame.mixer.music.play(-1) #play tiene tres parametros (-1 es infinito)
    ...
    while is_runnig:
            ...
            if event.type == KEYDOWN:
                if event.key == K_f:
                    if rafaga:
                        ...
                        lasers.append(generate_block(None, midtop[0] - laser_width // 2, midtop[1] - laser_height, 
                                                laser_width, laser_height, GREEN, speed_y= speed_laser))
                    else:
                        if not laser:
                            midtop = block["rect"].midtop
                            laser_width, laser_height = size_laser
        ...
        #Si existe el laser nos fijamos si colisiona con algun asteroide:
        if rafaga:
            for laser in lasers[:]:
                colision = False
                for coin in coins [ : ]:
